...
/*
	Registration Page:
		...
		function viewStudentStats(id, toggle) {
			if(toggle == '0'){
				document.getElementById(id).setAttribute("style","height:0px");
	...
	Football Season Stats Page:
		...
		function loadStatsPage() {
			...
			for(row_counter = 2; row_counter < table.rows.length; row_counter++){
				cell_1 = table.rows[row_counter].cells[2].innerHTML;
				cell_2 = table.rows[row_counter].cells[3].innerHTML;
				if(parseInt(cell_1) > parseInt(cell_2)){
					table.rows[row_counter].cells[4].innerHTML = "CU Boulder";
					var num = document.getElementById("wins").innerHTML;
					num++;
					document.getElementById("wins").innerHTML = num;
				}
				else if(parseInt(cell_1) < parseInt(cell_2)){
					table.rows[row_counter].cells[4].innerHTML = table.rows[row_counter].cells[1].innerHTML;
					var num = document.getElementById("losses").innerHTML;
					num++;
					document.getElementById("losses").innerHTML = num;
				}
			}
			document.getElementById().onload = null;
		}
*/
	...
	Football Player Information Page
		...
		function loadPlayersPage() {
			...
			for(counter = 0; counter< 4; counter++){
				...
				insert = insert + "<a class = 'dropdown-item' href='#' onclick = 'switchPlayers("+counter+")'>"+name+"</a>";
			}
		...
		function switchPlayers(playerNum){
			...
			var p =Math.round(players[playerNum].pass_yards/players[playerNum].games_played);
			var r =Math.round(players[playerNum].rushing_yards/players[playerNum].games_played);
